"""
db_config_ctx.py — DB connections for market_context service.

Two connection types:
  - get_main_conn_ro() → trading (read-only, autocommit)
  - get_main_conn_rw() → trading (read/write for market_context table)
"""
import os
import psycopg2
...
def get_main_conn_rw(autocommit=False):
    """Connect to trading DB (read/write for market_context table)."""
    conn = psycopg2.connect(
        host=os.getenv('MAIN_DB_HOST', 'localhost'),
        port=int(os.getenv('MAIN_DB_PORT', '5432')),
        dbname=os.getenv('MAIN_DB_NAME', 'trading'),
